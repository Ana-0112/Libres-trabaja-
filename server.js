...
app.post('/api/guardar-token', async (req, res) => {
    ...
    try {
        ...
        console.log("🔥 TOKEN GUARDADO:", user.email);
...
app.get('/api/vacantes/reclutador/:email', async (req, res) => {
    ...
    try {
        ...
        console.log("ERROR RECLUTADOR VACANTES:", e);

        res.status(500).json({
            error: "Error reclutador vacantes"
        });
...
app.post('/api/vacantes/postular', async (req, res) => {
    ...
    try {
        ...
        console.log("ERROR POSTULACION:", e);
...
app.get('/api/postulaciones/usuario/:email', async (req, res) => {
    ...
    try {
        ...
        const data = await Postulacion.find({
            candidatoEmail: email
        });
